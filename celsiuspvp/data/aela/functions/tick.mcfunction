...
execute as @a if entity @s[nbt={SelectedItem:{id:"minecraft:shield",tag:{Unbreakable:1}}}] run item replace entity @s weapon.mainhand with shield
execute as @a if entity @s[nbt={Inventory:[{Slot:-106b,id:"minecraft:shield",tag:{Unbreakable:1}}]}] run item replace entity @s weapon.offhand with shield