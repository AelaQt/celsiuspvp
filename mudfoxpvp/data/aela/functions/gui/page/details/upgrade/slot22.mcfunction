...
execute if entity @s[tag=noItem] run item replace entity @s container.22 with white_stained_glass_pane{GUI:1,display:{Name:'{"text":"No item selected","color":"dark_gray","italic":false}'}}

execute if entity @s[tag=!noUpgrade,tag=!noMoney,tag=!canUpgrade,tag=!noItem] run tag @s add noInfo

execute if entity @s[tag=noInfo] run item replace entity @s container.22 with white_stained_glass_pane{GUI:1,display:{Name:'{"text":"No information","color":"dark_gray","italic":false}'}}
